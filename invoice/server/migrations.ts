...
export const archiveMigration = {
  ...
  sql: `
    CREATE TABLE invoice_records (
      ...
      initial_payload_hash text NOT NULL,
      UNIQUE (owner_id, idempotency_key),
      CHECK ((status = 'draft' AND issued_at IS NULL AND pdf_key IS NULL AND pdf_sha256 IS NULL AND pdf_bytes IS NULL AND template_version IS NULL AND issued_from_revision IS NULL)
        OR (status = 'issued' AND issued_at IS NOT NULL AND pdf_key IS NOT NULL AND pdf_sha256 ~ '^[0-9a-f]{64}$' AND pdf_bytes > 0 AND template_version IS NOT NULL AND issued_from_revision IS NOT NULL))
    );
    CREATE INDEX invoice_owner_list ON invoice_records(owner_id, created_at DESC, id DESC);
    CREATE UNIQUE INDEX invoice_owner_reference ON invoice_records(owner_id, (data->>'reference')) WHERE status = 'issued';
    CREATE TABLE invoice_revisions (
...
export async function migrate(database: pg.Pool) {
  ...
  try {
    ...
    const legacyApplied = applied.rows.some(
      (row) => row.version === legacyMigration.version,
    )
    if (
      legacyApplied &&
      !applied.rows.some((row) => row.version === archiveMigration.version)
    )
      throw new Error(
        'Legacy self-hosted invoice schema detected. Refusing migration without an explicit account-ID mapping; do not reassign owners or drop invoice data.',
      )
    const existingTables = await client.query<{ exists: boolean }>(
      "SELECT to_regclass('public.invoice_records') IS NOT NULL AS exists",
    )
    if (!applied.rows.length && existingTables.rows[0].exists)
      throw new Error(
        'Untracked legacy invoice schema detected. Refusing migration without an explicit account-ID mapping.',
      )
    for (const migration of migrations) {
      if (migration.version === legacyMigration.version) continue
